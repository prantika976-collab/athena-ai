...
async function loadConversation(id) {
  ...
  const res = await fetch(
    `https://athena-backend-c8co.onrender.com/conversations/${id}/messages`,
    { headers: authHeader() }
  );
...
document.getElementById("fileInput")
  ?.addEventListener("change", async (e) => {
    ...
    const res = await fetch("https://athena-backend-c8co.onrender.com/ai/exam", {
      method: "POST",
      headers: authHeader(),
      body: formData
    });
...
document.getElementById("sendBtn")
  ?.addEventListener("click", async () => {
    ...
    try {
      let endpoint = "https://athena-backend-c8co.onrender.com/ai/chat";
      let payload = {
        userMessage: text,
        profile: userProfile,
      ...
      if (currentMode === "mentor") {
        endpoint = "https://athena-backend-c8co.onrender.com/ai/mentor";
        payload = {
          userMessage: text,
      ...
      if (currentMode === "competition") {
        endpoint = "https://athena-backend-c8co.onrender.com/ai/competition";
        payload = {
          userMessage: text,
          profile: userProfile,
          conversationId: currentConversationId
      ...
      if (currentMode === "exam") {
        endpoint = "https://athena-backend-c8co.onrender.com/ai/exam";
        payload = {
          userMessage: text,
          profile: userProfile,
      ...
      if (currentMode === "assignment") {
        endpoint = "https://athena-backend-c8co.onrender.com/ai/assignment";
        payload = {
          userMessage: text,
          profile: userProfile,
          conversationId: currentConversationId
